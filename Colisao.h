...
{
    /**
    * cria uma caixa de colis�o atrav�s dos maiores e menores pontos em cada eixo.
    */
    if(!modelo)
        exit(1);
    int i =0;
...
{
    /**
    * A caixa de colis�o � montada a partir do ponto <0,0,0>.. os meteoros tem uma posicao diferente
    * dessa entao, para eles, deve-se calcular o valor padr�o novamente.
    */
    int i;
    for(i =0; i <8; i++)
    {
        c->pontos[i].x+= posicao.x;//'translada' a caixa
        c->pontos[i].y+= posicao.y;//'translada' a caixa
        c->pontos[i].z+= posicao.z;//'translada' a caixa
    }

}

//atualiza o estado dos tiros e meteoros, suas posi��es e visibilidade.
